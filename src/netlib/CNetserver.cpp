...
namespace cov1013
{
	CNetServer::CNetServer()
	{
		...
		CLogger::CLogger(L"../Logs_NetServer", eLOG_LEVEL_DEBUG);
		InitializeProfiler(L"../Profiling_NetServer", en_PROFILER_UNIT::eUNIT_NANO);
		WSADATA	wsa;
	...
	void CNetServer::Stop(en_STOP_TYPE eType)
	{
		switch (eType)
		{
	...
	bool CNetServer::SendPacket(const SESSION_ID SessionID, CPacket* pPacket)
	{
	...
	bool CNetServer::Disconnect(const SESSION_ID SessionID)
	{
		st_SESSION* pSession = m_Sessions[GET_SESSION_INDEX(SessionID)];
		InterlockedIncrement16(&pSession->IOCount);
	...
	const int CNetServer::GetSessionCount(void)
	{
		return m_SessionMax - m_Indexes.GetCapacity();
	}

	const int CNetServer::GetIndexesCount(void)
	{
		return m_Indexes.GetCapacity();
	}

	CNetServer::st_SESSION* CNetServer::NewSession(const SOCKET ClientSocket)
	{
	...
	bool CNetServer::ReleaseSession(st_SESSION* pSession)
	{
	...
	// 세션 연결 종료 (내부용)
	bool CNetServer::Disconnect(st_SESSION* pSession)
	{
		InterlockedIncrement16(&pSession->IOCount);
	...
	// WSASend 등록
	bool CNetServer::SendPost(st_SESSION* pSession)
	{
		do
		{
	...
	// 수신 완료 처리
	void CNetServer::RecvProc(st_SESSION* pSession, const DWORD dwTransferred)
	{
		int iResult;
		...
		while (true)
		{
			...
			pSession->RecvQ.Peek((char*)&Header, CPacket::eHEADER_LEN);
			...
			iResult = pSession->RecvQ.Get(pRecvPacket->GetBufferPtr(), CPacket::eHEADER_LEN + Header.Len);
			pRecvPacket->MoveWritePos(iResult - CPacket::eHEADER_LEN);	// 헤더 길이를 뺀 페이로드 사이즈 만큼 WritePos 이동
	...
	void CNetServer::SendProc(st_SESSION* pSession, const DWORD dwTransferred)
	{
	...
	unsigned int __stdcall CNetServer::AcceptThread(void* lpParam)
	{
		return ((CNetServer*)lpParam)->AcceptThread_Procedure();
	}

	int CNetServer::AcceptThread_Procedure(void)
	{
		SOCKET		ClientSocket;
		SOCKADDR_IN	ClientAddr;
		int			addrlen = sizeof(ClientAddr);
	...
	unsigned int __stdcall CNetServer::IOCPWorkerThread(void* lpParam)
	{
		return ((CNetServer*)lpParam)->IOCPWorkerThread_Procedure();
	}
	int CNetServer::IOCPWorkerThread_Procedure(void)
	{
		DWORD		dwTransferred;
