...
namespace cov1013
{
	...
	CGameServer::~CGameServer()
	{
		vector<CPlayer*>::iterator iter = m_Players.begin();
	...
	void CGameServer::Run(void)
	{
		...
		{
			pPlayer = (CPlayer*)_aligned_malloc(sizeof(CPlayer), en_CACHE_ALIGN);
			new (pPlayer) CPlayer();
			m_Players.push_back(pPlayer);
			SetSession(i, (CNetSession*)pPlayer);
		}
	...
	// 서버 컨트롤러
	///////////////////////////////////////////////////////////////////////////////////
	void CGameServer::Controler(void)
	{
	...
	// 
	///////////////////////////////////////////////////////////////////////////////////
	unsigned int __stdcall CGameServer::MonitorThread(void* lpParam)
	{
		return ((CGameServer*)lpParam)->MonitorThread_Procedure();
	}
	int CGameServer::MonitorThread_Procedure(void)
	{
		CCpuUsage	CpuUsage;
		CPDH		PDH(L"GameServer");
...
//#endif
	...
	unsigned int __stdcall CGameServer::HeartBeatThread(void* lpParam)
	{
		return ((CGameServer*)lpParam)->HeartBeatThread_Procedure();
	}
	int CGameServer::HeartBeatThread_Procedure(void)
	{
		while (m_bLoop)
		{
	...
	unsigned int __stdcall CGameServer::MonitorServerConnectThread(void* lpParam)
	{
		return ((CGameServer*)lpParam)->MonitorServerConnectThread_Procedure();
	}

	int CGameServer::MonitorServerConnectThread_Procedure(void)
	{
		WCHAR	BIND_IP[16];
		WCHAR	SERVER_IP[16];
		WORD	SERVER_PORT;
	...
	void CGameServer::LockPlayers(void)
	{
		AcquireSRWLockExclusive(&m_Players_srw);
	}

	void CGameServer::UnlockPlayers(void)
	{
	...
	//=================================================================================
	CGameServer::CMonitorClient::CMonitorClient()
	{
	}
	CGameServer::CMonitorClient::~CMonitorClient()
	{
	...
	void CGameServer::CMonitorClient::SendPacket_UpdateData(const int DataValues[], const int DataCount)
	{
		if (!m_bConnectedFlag)
		{
			return;
	...
	void CGameServer::CMonitorClient::OnEnterJoinServer(void)
	{
		//-------------------------------------------------------
		// 로그인 요청 패킷 보내기
		//-------------------------------------------------------
	...
	void CGameServer::CMonitorClient::OnLeaveServer(void)
	{
		m_bConnectedFlag = false;
	}

	void CGameServer::CMonitorClient::OnRecv(CPacket* pRecvPacket)
	{
	}

	void CGameServer::CMonitorClient::OnSend(int sendsize)
	{
	}

	void CGameServer::CMonitorClient::OnWorkerThreadBegin(void)
	{
	}

	void CGameServer::CMonitorClient::OnWorkerThreadEnd(void)
	{
	}

	void CGameServer::CMonitorClient::OnError(int errorcode, wchar_t* szMsg)
	{
	}

	void CGameServer::CMonitorClient::MakePacket_Login(CPacket* pSendPacket, const int ServerNo)
	{
		*pSendPacket << (WORD)en_PACKET_SS_MONITOR_LOGIN;
		*pSendPacket << ServerNo;
	}

	void CGameServer::CMonitorClient::MakePacket_DataUpdate(CPacket* pSendPacket, const BYTE DataType, const int DataValue, int TimeStamp)
	{
