...
namespace cov1013
{
	...
	private:
		bool PacketProcedure_Login(const SESSION_ID SessionID, PacketBuffer* pRecvPacket);
		bool PacketProcedure_SectorMove(const SESSION_ID SessionID, PacketBuffer* pRecvPacket);
		bool PacketProcedure_ChatMessage(const SESSION_ID SessionID, PacketBuffer* pRecvPacket);
		bool PacketProcedure_Heartbeat(const SESSION_ID SessionID);

		void MakePacket_Login(PacketBuffer* pSendPacket, const BYTE byStatus, const INT64 AccountNo);
		void MakePacket_SectorMove(PacketBuffer* pSendPacket, const INT64 AccountNo, const WORD shSectorX, const WORD shSectorY);
		void MakePacket_ChatMessage(PacketBuffer* pSendPacket, const INT64 AccountNo, const WCHAR* szID, const WCHAR* szNickName, const WORD shChatMessageLen, const WCHAR* szChatMessage);

		bool JobProcedure_Connect(const SESSION_ID SessionID);
		bool JobProcedure_Disconnect(const SESSION_ID SessionID);
		bool JobProcedure_RecvMessage(const SESSION_ID SessionID, PacketBuffer* pRecvPacket);
		...
		void SendPacket_Around(st_CLIENT* pClient, PacketBuffer* pSendMessage, const bool bSendMe = false);
		void SendPacket_SectorOne(const int iX, const int iY, PacketBuffer* pSendMessage, const SESSION_ID ExceptSessionID = df_INVALID_SESSION_ID);
	...
	private:
		...
		MemoryPool<st_UPDATE_JOB>				m_UpdateJobPool = MemoryPool<st_UPDATE_JOB>(0);
		//CMemoryPool<st_REDIS_JOB>				m_RedisJoFbPool = CMemoryPool<st_REDIS_JOB>(0);

		CDBConnector_TLS*						m_pDBConnector;

		LockFreeQueue<st_UPDATE_JOB*>			m_UpdateJobQueue;
