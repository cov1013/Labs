...
namespace cov1013
{
	PDH::PDH(const WCHAR* szProcessName)
	{
		WCHAR szBuffer[1024];
	...
	void PDH::Collect(void)
	{
		PdhCollectQueryData(m_szQuery);
	}

	const LONGLONG PDH::GetProcessCommitMemory(void)
	{
	...
	const LONGLONG PDH::GetProcessNonPagedMemory(void)
	{
		PDH_FMT_COUNTERVALUE CounterValue;
	...
	const LONGLONG PDH::GetAvailableMemory(void)
	{
		PDH_FMT_COUNTERVALUE CounterValue;
	...
	const LONGLONG PDH::GetNonPagedMemory(void)
	{
		PDH_FMT_COUNTERVALUE CounterValue;
	...
	const LONGLONG PDH::GetRecvBytes(void)
	{
	...
	const LONGLONG PDH::GetSendBytes(void)
	{
	...
	const LONGLONG PDH::GetTCPv4Retransmitted(void)
	{
		PDH_FMT_COUNTERVALUE CounterValue;
	...
	const double PDH::GetPageFaults(void)
	{
		PDH_FMT_COUNTERVALUE CounterValue;
	...
	bool PDH::AddNetworkInterfaces(void)
	{
		bool	bErr = false;
		WCHAR* szCur = NULL;
