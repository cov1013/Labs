...
namespace cov1013
{
	...
	private:
		...
		void							PacketProcedure(const SESSION_ID SessionID, PacketBuffer* pRecvPacket);
		bool							PacketProcedure_Login(const SESSION_ID SessionID, PacketBuffer* pRecvPacket);
		void							MakePacket_ResLogin(PacketBuffer* pSendPacket, const INT64 AccountNo, const BYTE Status, const WCHAR* ID, const WCHAR* NickName, const WCHAR* GameServerIP, const USHORT	GameServerPort, const WCHAR* ChatServerIP, const USHORT ChatServerPort);
